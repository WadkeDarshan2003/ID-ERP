...
export const migrateLegacyFinancials = async (clearLegacy = true) => {
  try {
    console.log('🔄 Starting legacy financials migration...');
    ...
    for (const project of projects) {
      if (Array.isArray(project.financials) && project.financials.length > 0) {
        projectsWithData++;
        console.log(`📦 Project "${project.name}" has ${project.financials.length} legacy transactions`);
        ...
        for (const record of project.financials) {
          ...
          try {
            ...
            migratedCount++;
            console.log(`✅ Migrated transaction: ${record.description}`);
          } catch (e) {
        ...
        if (clearLegacy) {
          try {
            ...
            console.log(`🗑️ Cleared legacy financials for project ${project.name}`);
          } catch (e) {
    ...
    console.log(`✅ Migration complete: ${migratedCount} transactions from ${projectsWithData} projects migrated`);
