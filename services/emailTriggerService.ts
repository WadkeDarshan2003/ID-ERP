...
export const sendTaskCreationEmail = async (
  ...
  try {
    ...
    if (result.success) {
      console.log(`✅ Task creation email sent to ${assignee.name}`);
    } else {
    ...
    if (
      ...
      if (assignee?.email) {
        try {
          ...
          if (result.success) {
            console.log(`✅ 24-hour reminder sent to ${assignee.name} for task "${task.title}"`);
  ...
  const htmlContent = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #333;">
      ...
      <div style="background-color: #f0f9ff; border-left: 4px solid #0284c7; padding: 15px; margin: 20px 0; border-radius: 4px;">
        ...
        <p style="margin: 5px 0;"><strong>Added on:</strong> ${new Date().toLocaleDateString('en-IN')}</p>
      </div>
...
export const sendDocumentApprovalEmail = async (
  ...
  try {
    ...
    if (result.success) {
      console.log(`✅ Document approval email sent to ${recipient.name}`);
    } else {
...
export const sendTaskApprovalEmail = async (
  ...
  try {
    ...
    if (result.success) {
      console.log(`✅ Task approval email sent to ${recipient.name}`);
    } else {
